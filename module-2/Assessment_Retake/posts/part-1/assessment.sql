...
SELECT created, COUNT(created) FROM posts GROUP BY created

-- Select the user's first_name, last_name, and email and the name of the post for all posts created after Oct. 1st 2019
SELECT users.first_name, users.last_name, users.email, posts.name FROM users JOIN posts ON users.id = posts.user_id WHERE posts.created > '2019-10-01'