...
SELECT added, COUNT(added) FROM items GROUP BY added

-- Select the cart's username and created date and the item's name for all carts created in the month of Sept. 2019
SELECT username, created, items.name FROM carts JOIN items ON items.id = carts.id WHERE items.added BETWEEN '2019-09-01' AND '2019-09-30'